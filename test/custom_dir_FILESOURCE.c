...
main()
{
	...
	/* In case old file is present; delete file. */
	unlink(filename);
...
#define SCENETYPE_VAL 250
	...
	if (!TIFFSetField( tif, EXIFTAG_FILESOURCE, FILESOURCE_VAL)) {
		fprintf (stderr, "Can't write EXIFTAG_FILESOURCE\n" );
		goto failure;
	}		
	if (!TIFFSetField( tif, EXIFTAG_SCENETYPE, SCENETYPE_VAL)) {
		fprintf (stderr, "Can't write EXIFTAG_SCENETYPE\n" );
		goto failure;
	}		
	...
	if( u8 != FILESOURCE_VAL) {
		fprintf (stderr, "got wrong FILESOURCE value %d instead of %d.\n", u8, FILESOURCE_VAL);
		goto failure;
	}

	if (!TIFFGetField( tif, EXIFTAG_SCENETYPE, &u8) ) {
		fprintf (stderr, "reading EXIFTAG_SCENETYPE failed.\n" );
		goto failure;
	}
	if( u8 != SCENETYPE_VAL) {
		fprintf (stderr, "got wrong EXIFTAG_SCENETYPE value %d instead of %d.\n", u8, SCENETYPE_VAL);
		goto failure;
	}
